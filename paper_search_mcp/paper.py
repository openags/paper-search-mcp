...
class Paper:
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            'doi': self.doi,
            'published_date': self.published_date.isoformat() if isinstance(self.published_date, datetime) else (str(self.published_date) if self.published_date else ''),
            'pdf_url': self.pdf_url,
            'url': self.url,
            'source': self.source,
            'updated_date': self.updated_date.isoformat() if isinstance(self.updated_date, datetime) else (str(self.updated_date) if self.updated_date else ''),
