...
check_platform("arxiv.search", lambda: ArxivSearcher().search(QUERY, max_results=MAX_RESULTS))

# ── PubMed ───────────────────────────────────────────────────────────────────
print("\n[2] PubMed")
from paper_search_mcp.academic_platforms.pubmed import PubMedSearcher
check_platform("pubmed.search", lambda: PubMedSearcher().search(QUERY, max_results=MAX_RESULTS))
...
check_platform("biorxiv.search", lambda: BioRxivSearcher().search("bioinformatics", max_results=MAX_RESULTS, days=30))
...
check_platform("medrxiv.search", lambda: MedRxivSearcher().search("infectious_diseases", max_results=MAX_RESULTS, days=30))
...
check_platform("google_scholar.search", lambda: GoogleScholarSearcher().search(QUERY, max_results=MAX_RESULTS), optional=True)

# ── IACR ─────────────────────────────────────────────────────────────────────
print("\n[6] IACR  (fetch_details=False for speed)")
from paper_search_mcp.academic_platforms.iacr import IACRSearcher
check_platform("iacr.search", lambda: IACRSearcher().search("cryptography", max_results=MAX_RESULTS, fetch_details=False))

# ── Semantic Scholar ──────────────────────────────────────────────────────────
print("\n[7] Semantic Scholar")
from paper_search_mcp.academic_platforms.semantic import SemanticSearcher
check_platform("semantic.search", lambda: SemanticSearcher().search(QUERY, max_results=MAX_RESULTS), optional=True)

# ── CrossRef ──────────────────────────────────────────────────────────────────
print("\n[8] CrossRef")
from paper_search_mcp.academic_platforms.crossref import CrossRefSearcher
check_platform("crossref.search", lambda: CrossRefSearcher().search(QUERY, max_results=MAX_RESULTS))

# ── 9. PubMed Central (PMC) ──────────────────────────────────────────────────────────────
print("\n[9] PubMed Central (PMC)")
from paper_search_mcp.academic_platforms.pmc import PMCSearcher
check_platform("pmc.search", lambda: PMCSearcher().search(QUERY, max_results=MAX_RESULTS), optional=True)

# ── 10. CORE ─────────────────────────────────────────────────────────────────────────────
print("\n[10] CORE  (requires API key for full functionality)")
from paper_search_mcp.academic_platforms.core import CORESearcher
check_platform("core.search", lambda: CORESearcher().search(QUERY, max_results=MAX_RESULTS))

# ── 11. Europe PMC ───────────────────────────────────────────────────────────────────────
print("\n[11] Europe PMC  (biomedical literature)")
from paper_search_mcp.academic_platforms.europepmc import EuropePMCSearcher
check_platform("europepmc.search", lambda: EuropePMCSearcher().search(QUERY, max_results=MAX_RESULTS))

# ── 12. dblp ─────────────────────────────────────────────────────────────────────────────
print("\n[12] dblp  (computer science metadata)")
from paper_search_mcp.academic_platforms.dblp import DBLPSearcher
check_platform("dblp.search", lambda: DBLPSearcher().search(QUERY, max_results=MAX_RESULTS), optional=True)

# ── 13. OpenAIRE ─────────────────────────────────────────────────────────────────────────
print("\n[13] OpenAIRE  (European open access infrastructure)")
from paper_search_mcp.academic_platforms.openaire import OpenAiresearcher
check_platform("openaire.search", lambda: OpenAiresearcher().search(QUERY, max_results=MAX_RESULTS))

# ── 14. CiteSeerX ────────────────────────────────────────────────────────────────────────
print("\n[14] CiteSeerX  (computer science digital library)")
from paper_search_mcp.academic_platforms.citeseerx import CiteSeerXSearcher
check_platform("citeseerx.search", lambda: CiteSeerXSearcher().search(QUERY, max_results=MAX_RESULTS), optional=True)

# ── 15. DOAJ ─────────────────────────────────────────────────────────────────────────────
print("\n[15] DOAJ  (open access journals)")
from paper_search_mcp.academic_platforms.doaj import DOAJSearcher
check_platform("doaj.search", lambda: DOAJSearcher().search(QUERY, max_results=MAX_RESULTS))

# ── 16. BASE ─────────────────────────────────────────────────────────────────────────────
print("\n[16] BASE  (OAI-PMH academic repositories)")
from paper_search_mcp.academic_platforms.base_search import BASESearcher
check_platform("base.search", lambda: BASESearcher().search(QUERY, max_results=MAX_RESULTS), optional=True)

# ── 17. Zenodo ───────────────────────────────────────────────────────────────────────────
print("\n[17] Zenodo  (open repository)")
from paper_search_mcp.academic_platforms.zenodo import ZenodoSearcher
check_platform("zenodo.search", lambda: ZenodoSearcher().search(QUERY, max_results=MAX_RESULTS))

# ── 18. HAL ──────────────────────────────────────────────────────────────────────────────
print("\n[18] HAL  (French open archive)")
from paper_search_mcp.academic_platforms.hal import HALSearcher
check_platform("hal.search", lambda: HALSearcher().search(QUERY, max_results=MAX_RESULTS))

# ── 19. SSRN ─────────────────────────────────────────────────────────────────────────────
print("\n[19] SSRN  (social sciences; best-effort full-text)")
from paper_search_mcp.academic_platforms.ssrn import SSRNSearcher
check_platform("ssrn.search", lambda: SSRNSearcher().search(QUERY, max_results=MAX_RESULTS), optional=True)
...
check_platform("unpaywall.search", lambda: UnpaywallSearcher().search("10.1038/nature12373", max_results=1), optional=True)
