...
class TestPaperSearchApi(unittest.TestCase):
    def test_all_sources_include_new_platforms(self):
        ...
        self.assertIn("citeseerx", api.ALL_SOURCES)
        self.assertIn("doaj", api.ALL_SOURCES)
        self.assertIn("base", api.ALL_SOURCES)
        self.assertIn("zenodo", api.ALL_SOURCES)
        self.assertIn("hal", api.ALL_SOURCES)
        self.assertIn("ssrn", api.ALL_SOURCES)
        self.assertIn("unpaywall", api.ALL_SOURCES)

    def test_parse_sources_with_new_platforms(self):
        parsed = api._parse_sources("dblp,doaj,base,zenodo,hal,ssrn,unpaywall,invalid")
    ...
    def test_search_arxiv(self):
        ...
        result = asyncio.run(api.search_arxiv("machine learning", max_results=10))
    ...
    def test_download_arxiv_from_search(self):
        ...
        # 先搜索 10 个结果
        search_results = asyncio.run(api.search_arxiv("machine learning", max_results=10))
